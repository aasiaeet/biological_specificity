...
# par(mfrow = c(3, 2))
#random
# hist(adata$X[,colnames(adata$X)=='BRAF'], breaks=123, main='BRAF', xlab='CPM')
# hist(adata$X[,colnames(adata$X)=='TP53'], breaks=123, main='TP53', xlab='CPM')
#mostly expressed
...
head(daft)
sum(ctype %in% names(tab[tab > 100]))
...
```
So, there are 5989 mRNAs that have exactly zero count which we can not do anything about them. Also, if nnz < minimum number of samples per cell type, we can not do anything about them. So we remove those mRNAs from further consideration. 

```{r , warning = FALSE}
f <- file.path(paths$scratch, 'TS_sub_blood_10p.Rda') 
if (file.exists(f)) {
  load(f)
} else {
  ind <- nnz > 5000
  subX <- sX[,ind]
  ctype <- subC
  save(subX, ctype, file = f)
}
```

We order that mRNAs according to the number of zeros they have, i.e., miRs with most zeros are processed first:

```{r results}
library(Matrix)
nnz <- tabulate(subX@j + 1L, ncol(subX)) #nnz per column (mRNA)
tempd <- t(as.matrix((subX[,order(nnz)])))
nonzeros <- sort(nnz)
```
...
library(tictoc)
set.seed(12345)
beansdir<- file.path(paths$scratch, "BeanMixture-TS-Blood-10percent")
if (!file.exists(beansdir)) dir.create(beansdir)
res <- 300
cohortColors <- viridis(length(unique(ctype)))

f <- file.path(paths$scratch, 'allStat-TS-Blood_10p.rda')
g <- file.path(paths$scratch, 'brlist-TS-Blood_10p.rda')
h <- file.path(paths$scratch, 'postlist-TS-Blood_10p.rda')
...
if (file.exists(f)) {
  ...
  allStat <- c()
  for (I in 1:length(nonzeros)) { #for each mRNA
  # for (mRNA in c('ARPC2', 'ATP5F1E', 'RPL12', 'RPL36', 'YWHAZ', 'ZFAND5')){
    tic()
    print(I)
    mRNA <- rownames(tempd)[I]
    
    # I <- which(rownames(tempd) == mRNA)
    
    # make a binary vector of zero or not.
    X <- c("NonZero", "Zero")[1 + 1*(tempd[I,] == 0)]
    cat("Working on", mRNA, "\n", file = stderr())
    tab <- table(ctype, X)
    K <- tab[,1]
    N <- apply(tab, 1, sum)
    ...
    ourstats <- c(mRNA, nonzeros[I], mt, post$fisherC)
